...
export class PBPhraseTypeOrGroupsFuzzySuggestModal extends FuzzySuggestModal<
  PBItem | GroupItem
> {
  app: App;
  plugin: PBPlugin;
  ...
  updateSuggestionElWithDesc(
    item: FuzzyMatch<PBItem | GroupItem>,
    el: HTMLElement
  ) {
    if (item.item.desc) {
    ...
    if (item.phraseType) {
      if (!evt.shiftKey) {
        new PBPhrasesFuzzySuggestModal(
          this.app,
          this.plugin,
          (item as PBItem).phrases,
          this.settings
        ).open();
      } else {
        const randI = Math.floor(
          Math.random() * ((item as PBItem).phrases.length - 1)
        );
      ...
      const filteredPB = this.pb.filter((pbItem) =>
        pbItem.groups.some((group) => group.name === item.name)
      );
      console.log({ filteredPB });
      this.close();
      new PBPhraseTypeFuzzySuggestModal(
        this.app,
        this.plugin,
        filteredPB,
        this.settings
